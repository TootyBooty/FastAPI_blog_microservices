...
from api.schemas.user import UserShow, UserOut, UserUpdate, UserCreate, UserRole
from api.urls import user_base_url, user_all_url
...
async def test_user(
    authorized_user = Depends(get_user_data_from_token),
                    session = Depends(get_aiohttp_session)):
    return await make_request(session=session, url=f'{user_base_url}/test_user', method='get',
