...
class BlogRepository(BaseRepository):
    ...
    async def get_posts(self, limit:int = None) -> list[Post]:
        cursor = self.collection.find({}, {'_id': 0, 'post_id': '$_id',
                                                'author': 1, 'title': 1,'content': 1,'comments': 1,'created_at': 1, 'updated_at': 1})
        res = []
    ...
    async def add_comment(self, author, post_id, comment:CommentIn, current_time) -> Union[CommentOut, None]:
    ...
    async def delete_comment(self, post_id, comment_id) -> Union[CommentOut, None]:
        res = await self.collection.update_one({'_id':post_id}, {'$pull': {'comments': {'comment_id': comment_id}}})
        if bool(res.modified_count):
